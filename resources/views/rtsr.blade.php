...
<body>
    ...
    <script>
        ...
        function updateStatistics(data) {
            const counts = data.map(entry => entry.peoplecount);
            const lowestCount = Math.min(...counts);
            const greatestCount = Math.max(...counts);

            const entryOfLowestCount = data.find(entry => entry.peoplecount === lowestCount);
            const entryOfGreatestCount = data.find(entry => entry.peoplecount === greatestCount);
            ...
            $('#averageCount').text(averageCount.toFixed(2));
